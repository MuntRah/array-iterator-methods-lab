...
let veryOldInventors = inventors.filter((n) => n.year >= 1500 && n.year < 1600);
...
let inventorNames = inventors.map((n) => {
...
let sortedByBirthYear = inventors.sort((y, p) => y.year - p.year);
...
let inventorNamedAda = inventors.find((n) => n.first === "Ada");
...
let isAdultPresent = devs.some((age) => 2024 - age.year >= 18);
...
let isEveryone19OrOlder = devs.every((age) => {
  age.year >= 19;
});
...
let commentById = comments.find((i) => i.id === 823423);
...
let idx = comments.findIndex((n) => n.id === 123523);
...
const age = inventors.reduce((count, cummul) => {
  return (count += cummul.passed - cummul.year);
}, 0);

// Enter your solution code here:

// Check your return value:
// console.log("Bonus 1 My Result: ", age);
